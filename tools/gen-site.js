...
function clientData() {
  const D = DESTINATIONS.map((d) => {
    ...
    return {
      ...
      count: loc.length,
      keywords: d.keywords || autoKw(d.name, d.state)
    };
  });
  const L = LISTINGS.map((l) => ({
    ...
    keywords: l.keywords || autoKw(l.city, l.state)
  }));
    ...
    <section class="hero hero--photo">
      ...
      <div class="container">
        ...
        <div class="hero-stats">
          ...
          <div class="stat-card"><div class="stat-num" data-count="100" data-suffix="%">100%</div><div class="stat-label">Free to list your BNB</div></div>
        </div>
      </div>
    </section>
  ...
  <main>
    ...
    <section class="section-alt">
      <div class="container">
        ...
        <p class="section-sub" style="text-align:center;">Don't see your city? <a href="contact.html">Suggest a destination</a> or <a href="list-your-bnb.html">list your BNB free</a> — we add new locations automatically as listings come in.</p>
      </div>
    </section>
  </main>
  ` + tail());
...
function genDestinationPages() {
  for (const d of DESTINATIONS) {
    ...
    const keywords = (d.keywords || autoKw(d.name, d.state)).join(", ");
...
function genBnbPages() {
  for (const l of LISTINGS) {
    ...
    const keywords = (l.keywords || autoKw(l.city, l.state)).join(", ");
    const facts = [
    ...
    write(`bnbs/${l.slug}.html`, head({
      ...
      desc: `${blurb(l)} ★ ${l.rating || "—"} (${l.reviews || 0} reviews). Check live pricing and book this ${(l.type || "airbnb").toLowerCase()} in ${l.city}, ${l.state} on Airbnb.`,
...
function genTierPosts() {
  ...
  <meta name="keywords" content="${(d.keywords || autoKw(d.name, d.state)).join(", ")}">
  ` + tail("../"));
    }
  }
}
...
function genDetailsPosts() {
  ...
  <meta name="keywords" content="${(l.keywords || autoKw(l.city, l.state)).join(", ")}">
  ` + tail("../"));
  }
}
